    ...
    return (
        <div className="h-screen bg-popover text-foreground flex flex-col overflow-hidden">
            ...
            <div className="flex flex-1 overflow-hidden max-w-7xl mx-auto w-full border-x border-border bg-background">
                {/* Sidebar */}
                <div className={`${showMobileList ? 'flex' : 'hidden'} md:flex w-full md:w-80 bg-popover border-r border-border flex-col shrink-0`}>
                            ...
                            <div ref={searchRef} className="space-y-2 relative animate-in fade-in slide-in-from-top-2 duration-200">
                                <div className="flex gap-2">
                                    ...
                                    <button
                                        ...
                                        className="bg-primary hover:bg-primary/90 text-white text-sm px-4 py-2 rounded-xl transition font-medium"
                                    >Start</button>
                                </div>
                                {newUserError && <p className="text-xs text-destructive pl-1">{newUserError}</p>}

                                {/* Autocomplete Dropdown */}
                                {showSuggestions && suggestions.length > 0 && (
                                    <div className="absolute top-full left-0 right-0 mt-2 bg-popover border border-border rounded-xl shadow-2xl z-[60] overflow-hidden animate-in fade-in zoom-in-95 duration-100">
                                        <div className="py-1">
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                        {loading ? (
                            ...
                            <div className="px-6 py-12 text-center">
                                ...
                                <p className="text-xs text-muted-foreground mt-1">Start a new one by clicking the + button</p>
                            </div>
                        ) : (
                                ...
                                {conversations.map(conv => (
                                    ...
                                    >
                                        <div className="relative shrink-0">
                                            ...
                                            {conv.unreadCount > 0 && (
                                                <span className="absolute -top-1 -right-1 bg-primary text-white text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center border-2 border-popover">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-baseline mb-0.5">
                                                <p className={`text-sm font-semibold truncate ${activeConvId === conv.id ? 'text-primary' : 'text-foreground'}`}>
                                                    {conv.otherUsername}
                                                </p>
                                            </div>
                                            {conv.lastMessage && (
                ...
                <div className={`${!showMobileList ? 'flex' : 'hidden'} md:flex flex-1 flex flex-col bg-background overflow-hidden`}>
                    {!activeConvId ? (
                        ...
                        <>
                            {/* Chat Header */}
                            <div className="bg-popover/80 backdrop-blur-md border-b border-border px-4 md:px-6 py-3 flex items-center justify-between shrink-0 z-10">
                                <div className="flex items-center gap-3 md:gap-4">
                                    ...
                                    <div className="w-10 h-10 rounded-full bg-primary flex items-center justify-center text-sm font-bold text-white shadow-lg shadow-primary/10">
                                        {activeConv?.otherUsername[0].toUpperCase()}
                                    </div>
                                    <div>
                                        <button
                                            onClick={() => navigate(`/profile/${activeConv?.otherUsername}`)}
                                            className="text-base font-bold text-foreground hover:text-primary transition block text-left"
                                        >
                                            {activeConv?.otherUsername}
                                        </button>
                                        <span className="text-[10px] text-primary font-medium tracking-wider uppercase">Active Chat</span>
                                    </div>
                                </div>
                                <div className="flex gap-2">
                                            ...
                                            <div className={`max-w-[75%] lg:max-w-[60%] flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                                                <div className={`px-4 py-2.5 rounded-2xl text-[13.5px] leading-relaxed break-words shadow-sm
                                                    ${isMe
                                                        ...
                                                        : 'bg-popover text-foreground rounded-bl-none border border-border'}`}
                                                >
                                                    <p>{msg.content}</p>
                                                </div>
                                                {showAvatar && (
